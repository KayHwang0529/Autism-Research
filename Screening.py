...
model = Sequential(
    [tf.keras.Input(shape=(17,)), Dense(25, activation='relu', kernel_regularizer = tf.keras.regularizers.L2(0.1), name = 'layer1'), Dense(15, activation='relu', name = 'layer2'),Dense(1, activation='sigmoid', name = 'layer3')]
)


#specifices loss function (avg. gives cost function)________________
model.compile(
    loss = BinaryCrossentropy(), #from_logits=True inside parenthesis if cross entropy
